...
class CastedLinear(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return F.linear(input, self.weight.to(self.cast_to), bias=self.bias.to(self.cast_to) if self.bias is not None else None)
...
class Attention(nn.Module):
    def __init__(self, hidden_size, head_dim, num_heads, num_key_value_heads=None, causal=False, dtype=torch.float32):
        super().__init__()
...
class SwiGLU(nn.Module):
    def __init__(self, hidden_size: int, expansion: float, dtype=torch.float32):
        super().__init__()
